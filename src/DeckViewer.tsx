...
export default function DeckViewer(): JSX.Element {
  ...
  return (
    <div className="tl">
      <label htmlFor="deck-code-input">Deck Code</label>
      <br />
      <input
        className="pa3 w-100"
        id="deck-code-input"
...
function ValidDeckList({
  deckCode,
  deck,
  invalid,
}: ValidDeckListProps): JSX.Element {
  if (deckCode === "") {
    return <></>;
  }

  if (invalid) {
    return <span className="red">Please enter a valid deck code..</span>;
  } else {
    return (
      <div>
